...
class APIClient {
    ...
    class func codableAPIRequest<T>(showIndicator: Bool? = true, request: APIConfiguration, completionHandler: @escaping(_ result: T?) -> Void) where T:Codable {
        ...
        var parametersHeader = request.headers
        
        if languageBool {
            parametersHeader["lang"] = "ar"
        } else {
            parametersHeader["lang"] = "en"
        }
        AF.request(
            request.url,
    ...
    class func apiRequest(request: APIConfiguration, completionHandler : @escaping(_ result: NSDictionary?) -> Void) {
        ...
        AF.request(request.url, method: request.method, parameters: request.parameters, encoding: URLEncoding.default, headers: parametersHeader).validate(statusCode: 200..<500).responseJSON { response in
            ...
            print("---------------------------")
            print("Access Token \(SessionManager.getUserAccessToken() ?? "No Access Token")")   // Acccess Token
            print("---------------------------")
        ...
        AF.upload(multipartFormData: { multipartFormData in
            ...
            for (key, value) in request.parameters {
                multipartFormData.append(value .data(using: .utf8)!, withName: key)
                  ...
                  to: request.url,
                  method: request.method,
                  headers: parametersHeader).uploadProgress(closure: { progress in
            print(progress.fractionCompleted)
        }).responseJSON { response in
            Utilities.hideIndicatorView()
            print("adaf  \(response.response?.statusCode ?? 0)")
            switch response.response?.statusCode {
            case 401:
                guard let reponseDict = response.value as? NSDictionary, let message = reponseDict["message"] as? String else {
                    sessionExpiredAction(message: "Session expired. Please login again.")
